...
productBuy.addEventListener("click", () => {
  ...
  intervalWarning = setInterval(timeWarning, 60000);
...
function timeWarning(){
  ...
  let messaegOut = setTimeout(timeWarningOut, 10000);
}
...
//After 5 minutes show expired time alerte and reload page to the beginning
...
//VALIDATE PROFILE//
 ...
 //USERNAME VALIDATE REQUIRED AND 5-20 CHARACTERS WITHOUT BLANK SPACE//
    ...
    if(Cpassword.value === null || Cpassword.value === ""){
        ...
        errorCpassword.classList.remove("hidden");
...
var expressionFirstName = /^[a-zA-Z]+$/;
...
//ERROR MSG//
 ...
 //FIRST NAME VALIDATE REQUIRED MAX 20 //
    ...
    }else if(!expressionFirstName.test(firstName.value)){
        mensajeFname.push("First name cannot contain numbers");
    } 
    else {
      checkFirstName = true;
    }
...
}else if(!expressionFirstName.test(lastName.value)){
        mensajeLname.push("Last name cannot contain numbers");
    }else {
      checkLastName = true;
    }
...
document
  .querySelector(".shipping__options").addEventListener("click", event => {
    ...
    if (event.target.id === "shipment__3") {
      ...
      deliveryStart.innerHTML =
        month + " " + (day + 1) + ordinal + ", " + year + " at " + deliveryTimeStart;
        deliveryEnd.innerHTML = month + " " + (day + 1) + ordinal + ", " + year + " at " + deliveryTimeEnd + ".";
        deliveryDate.classList.remove("hidden");
    }
  });
...
let fileUpload = document.querySelector("#wrapper");

document.querySelector("#wrapper").addEventListener("input", ()=>{
  //console.log(fileUpload.files[0].name);
  document.querySelector(".loadedFile").innerHTML = fileUpload.files[0].name;
  document.querySelector(".loadedFileConfirm").classList.remove("hidden");
...
document.querySelector(".clear-input").addEventListener("click", () => {
  ...
  if (!profileSection.classList.contains("hidden")) {
    ...
    document.querySelector("#gift").click();
    document.querySelector(".container__delivery__date").classList.add("hidden");
    document.querySelector(".loadedFileConfirm").classList.add("hidden");
...
document.querySelector(".submit .next-input").addEventListener("click", () => {
  ...
  if (!profileSection.classList.contains("hidden")) {
    ...
    //document.querySelector(".contaienr__delivery__date").classList.add("hidden");
  }
});
